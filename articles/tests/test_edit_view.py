...
def test_article_edit_exists_at_desired_location(client, db, article):
...
def test_article_edit_view_name(client, db, article):
...
def test_edit_article_can_edit_article(client, db, article):
...
def test_article_edit_uses_correct_template(client, db, article):
    response = client.get(reverse("article_edit", kwargs={"pk": f"{article.pk}"}))
